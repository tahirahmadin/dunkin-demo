      ...
      <div className={`fixed right-0 top-0 h-full w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out ${
        ...
        <div className="p-6 bg-orange-50">
              ...
              <div className="flex items-center gap-1 text-sm text-gray-500">
                {latestAddress && (
                  <>
                    ...
                    <span className="line-clamp-1">{latestAddress.address}</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-3 shadow-sm">
            ...
            <p className="text-xs text-gray-500 mt-1">550 points until next reward</p>
          </div>
        </div>
            ...
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors ${
                  activeTab === tab.id ? 'bg-orange-50 text-orange-500' : 'hover:bg-gray-50'
                }`}
              >
          ...
          {activeTab === 'addresses' && (
            <div className="mt-4 px-4">
              {savedAddresses.map((addr) => (
                <div key={addr.id} className="bg-white p-3 rounded-xl mb-3 shadow-sm">
              ...
              {savedAddresses.length === 0 && (
                <p className="text-center text-gray-500 text-sm">No saved addresses</p>
              )}
            </div>
          )}
        </div>
